...
import com.yongheng.utils.NetworkUtils;
    ...
    private StringRequest requestForString(String url, int method, final boolean isShowToast, final IRequestCallback callback, boolean cache) {
        ...
        IpinStringRequest request = new IpinStringRequest(method, url, new Response.Listener<String>() {
            public void onResponse(String response) {
                ...
                if (callback != null) {
                    if (isShowToast) {
                        MyClient.getMyClient().getResponseErrorManager().handleErrorMsg(response);
                    }
            ...
            public void onErrorResponse(VolleyError error) {
                if (error != null) {
                    ...
                    if (error.networkResponse != null) {
                        ...
                        if (isShowToast) {
                            MyClient.getMyClient().getResponseErrorManager().handleErrorMsg(error.networkResponse.statusCode, "");
                        }
                    }
            ...
            request = new StringRequestJsonResult(method, url, new Response.Listener<JSONObject>() {
                ...
                public void onErrorResponse(VolleyError error) {
                    if (error != null) {
                        ...
                        } else if (!NetworkUtils.isNetworkAvailable(MainApplication.getContext()) && isShowToast) {
                            MyClient.getMyClient().getResponseErrorManager().handleErrorMsg(-6, "");
                        }
...
//                    StatManager.statEventNum(HttpRequestManager.this.mContext, "http_fail", map);
    ...
    private Request requestForPostWithJson(String url, Map<String, String> param, final boolean isShowToast, final IRequestCallback callback) {
